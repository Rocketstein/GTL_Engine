...
FRotator::FRotator(const FQuat &InQuat) noexcept : FRotator(InQuat.Rotator()) {}
...
FRotator FRotator::MakeFromEuler(const FVector3 &InEulerDegrees) noexcept
{
...
bool FRotator::operator!=(const FRotator &Other) const noexcept { return !(*this == Other); }

FRotator FRotator::operator-() const noexcept { return FRotator(-Pitch, -Yaw, -Roll); }
...
FVector3 FRotator::Euler() const noexcept { return FVector3(Roll, Pitch, Yaw); }

FVector3 FRotator::Vector() const noexcept
{
    const float PitchRadians = FMath::DegreesToRadians(Pitch);
    const float YawRadians = FMath::DegreesToRadians(Yaw);

    const float CosPitch = std::cos(PitchRadians);
    return FVector3(CosPitch * std::cos(YawRadians), CosPitch * std::sin(YawRadians),
                    std::sin(PitchRadians))
        .GetSafeNormal();
}

FVector3 FRotator::RotateVector(const FVector3 &InVector) const noexcept
{
    return Quaternion().RotateVector(InVector);
}

FVector3 FRotator::UnrotateVector(const FVector3 &InVector) const noexcept
{
...
{
    return NormalizeAxis(Pitch) == 0.0f && NormalizeAxis(Yaw) == 0.0f &&
...
float FRotator::GetManhattanDistance(const FRotator &Other) const noexcept
{
    return std::fabs(NormalizeAxis(Pitch - Other.Pitch)) +
           std::fabs(NormalizeAxis(Yaw - Other.Yaw)) + std::fabs(NormalizeAxis(Roll - Other.Roll));
}
...
FRotator FRotator::GetInverse() const noexcept { return Quaternion().Inverse().Rotator(); }

FQuat FRotator::Quaternion() const noexcept
{
    const FMatrix RotationMatrix = FMatrix::MakeRotationZ(FMath::DegreesToRadians(Yaw)) *
