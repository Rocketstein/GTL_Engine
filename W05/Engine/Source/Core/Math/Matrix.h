...
#include "Core/Math/SIMD/DXMathBridge.h"
...
struct FMatrix
{
  ...
  public:
    ...
    FMatrix(DirectX::FXMMATRIX InMatrix) noexcept { FDXMathBridge::StoreMatrix(*this, InMatrix); }
  ...
  public:
    ...
    bool operator==(const FMatrix &Other) const noexcept
    {
        ...
        {
            const DirectX::XMVECTOR ThisRow =
                DirectX::XMVectorSet(M[Row][0], M[Row][1], M[Row][2], M[Row][3]);
            const DirectX::XMVECTOR OtherRow = DirectX::XMVectorSet(
                Other.M[Row][0], Other.M[Row][1], Other.M[Row][2], Other.M[Row][3]);
    ...
    //======================================//
  public:
    DirectX::XMMATRIX ToXMMatrix() const noexcept { return FDXMathBridge::LoadMatrix(*this); }

    // 두 행렬이 허용 오차(Tolerance) 범위 내에서 같은지 비교함
    bool Equals(const FMatrix &Other, float Tolerance = 1.e-6f) const noexcept
    {
        const DirectX::XMVECTOR ToleranceVector = DirectX::XMVectorReplicate(Tolerance);
        for (int32 Row = 0; Row < 4; ++Row)
        {
            const DirectX::XMVECTOR ThisRow =
                DirectX::XMVectorSet(M[Row][0], M[Row][1], M[Row][2], M[Row][3]);
    ...
    FVector3 TransformVector(const FVector3 &V) const noexcept
    {
        FVector3 Result;
        FDXMathBridge::StoreVector(Result,
                                   DirectX::XMVector3TransformNormal(V.ToXMVector(), ToXMMatrix()));
        return Result;
    }

    // 위치 벡터를 현재 행렬로 변환함
    // 이동(Translation)을 포함하여 적용함
    FVector3 TransformPosition(const FVector3 &V) const noexcept
    {
        FVector3 Result;
        FDXMathBridge::StoreVector(Result,
                                   DirectX::XMVector3TransformCoord(V.ToXMVector(), ToXMMatrix()));
        return Result;
    }
    ...
    FVector4 TransformVector4(const FVector4 &V, const FMatrix &M) const noexcept
    {
        ...
        return FVector4(DirectX::XMVectorGetX(T), DirectX::XMVectorGetY(T),
                        DirectX::XMVectorGetZ(T), DirectX::XMVectorGetW(T));
    }

    // 현재 행렬의 이동(Translation) 성분을 반환함
    FVector3 GetOrigin() const noexcept { return FVector3(M[3][0], M[3][1], M[3][2]); }

    // 현재 행렬의 이동(Translation) 성분을 설정함
    void SetOrigin(const FVector3 &Origin) noexcept
    {
        M[3][0] = Origin.X;
        M[3][1] = Origin.Y;
        M[3][2] = Origin.Z;
    ...
    FVector3 GetScaledAxis(EAxis Axis) const noexcept
    {
        ...
        {
        case EAxis::X:
            return FVector3(M[0][0], M[0][1], M[0][2]);
        case EAxis::Y:
            return FVector3(M[1][0], M[1][1], M[1][2]);
        case EAxis::Z:
            return FVector3(M[2][0], M[2][1], M[2][2]);
        default:
    ...
    FVector3 GetUnitAxis(EAxis Axis) const noexcept { return GetScaledAxis(Axis).GetSafeNormal(); }
    ...
    {
        const FVector3 XAxis = GetScaledAxis(EAxis::X).GetSafeNormal(Tolerance);
        const FVector3 YAxis = GetScaledAxis(EAxis::Y).GetSafeNormal(Tolerance);
        const FVector3 ZAxis = GetScaledAxis(EAxis::Z).GetSafeNormal(Tolerance);
    ...
    FVector3 GetScaleVector() const noexcept
    {
        const DirectX::XMVECTOR XAxis = DirectX::XMVectorSet(M[0][0], M[0][1], M[0][2], 0.0f);
        const DirectX::XMVECTOR YAxis = DirectX::XMVectorSet(M[1][0], M[1][1], M[1][2], 0.0f);
        const DirectX::XMVECTOR ZAxis = DirectX::XMVectorSet(M[2][0], M[2][1], M[2][2], 0.0f);

        return FVector3(DirectX::XMVectorGetX(DirectX::XMVector3Length(XAxis)),
    ...
    FMatrix ApplyScale(const FVector3 &Scale) const noexcept { return MakeScale(Scale) * *this; }

    // 현재 행렬에 균일 스케일을 적용한 새 행렬을 반환함
    FMatrix ApplyScale(float Scale) const noexcept
    {
        return ApplyScale(FVector3(Scale, Scale, Scale));
    }
    ...
    FVector3 GetForwardVector() const noexcept { return GetUnitAxis(EAxis::X); }

    // 현재 행렬의 Right 방향 벡터를 반환함
    FVector3 GetRightVector() const noexcept { return GetUnitAxis(EAxis::Y); }

    // 현재 행렬의 Up 방향 벡터를 반환함
    FVector3 GetUpVector() const noexcept { return GetUnitAxis(EAxis::Z); }

    // 축 벡터와 위치를 이용하여 현재 행렬을 설정함
    void SetAxes(const FVector3 &XAxis, const FVector3 &YAxis, const FVector3 &ZAxis,
                 const FVector3 &Origin = FVector3::ZeroVector) noexcept
    {
        M[0][0] = XAxis.X;
        M[0][1] = XAxis.Y;
        M[0][2] = XAxis.Z;
    ...
    // 분해에 실패하면 false를 반환함
    bool Decompose(FVector3 &OutTranslation, FMatrix &OutRotation, FVector3 &OutScale,
                   float Tolerance = 1.e-8f) const noexcept
    {
        ...
        const FVector3 XAxis = GetScaledAxis(EAxis::X);
        const FVector3 YAxis = GetScaledAxis(EAxis::Y);
        const FVector3 ZAxis = GetScaledAxis(EAxis::Z);

        OutScale = FVector3(XAxis.Size(), YAxis.Size(), ZAxis.Size());
        ...
        const FVector3 UnitX = XAxis / OutScale.X;
        const FVector3 UnitY = YAxis / OutScale.Y;
        const FVector3 UnitZ = ZAxis / OutScale.Z;

        OutRotation = Identity;
        OutRotation.SetAxes(UnitX, UnitY, UnitZ, FVector3::ZeroVector);

        return true;
    }

    // 현재 행렬의 위치(Translation) 성분을 반환함
    FVector3 GetTranslation() const noexcept { return GetOrigin(); }

    // 현재 행렬의 위치(Translation) 성분을 설정함
    void SetTranslation(const FVector3 &Translation) noexcept { SetOrigin(Translation); }

    // 이동(Translation) 행렬을 생성함
    static FMatrix MakeTranslation(const FVector3 &Translation) noexcept
    {
    ...
    // 스케일(Scale) 행렬을 생성함
    static FMatrix MakeScale(const FVector3 &Scale) noexcept
    {
    ...
    {
        return MakeScale(FVector3(Scale, Scale, Scale));
    }

    // X축 방향 벡터를 기준으로 직교 기저 행렬을 생성함
    // 이 방향이 앞으로 향하는 X축이다. 라고 정하는 함수임
    static FMatrix MakeFromX(const FVector3 &XAxis) noexcept
    {
        const FVector3 X = XAxis.GetSafeNormal();
        if (X.IsNearlyZero())
        {
            return Identity;
        }

        const FVector3 UpCandidate =
            (std::fabs(X.Z) < 0.999f) ? FVector3::UpVector : FVector3::RightVector;

        const FVector3 Y = FVector3::CrossProduct(UpCandidate, X).GetSafeNormal();
        const FVector3 Z = FVector3::CrossProduct(X, Y).GetSafeNormal();
    ...
    static FMatrix MakeFromY(const FVector3 &YAxis) noexcept
    {
        const FVector3 Y = YAxis.GetSafeNormal();
        if (Y.IsNearlyZero())
        {
            return Identity;
        }
        ...
        const FVector3 UpCandidate =
            (std::fabs(Y.Z) < 0.999f) ? FVector3::UpVector : FVector3::ForwardVector;

        const FVector3 X = FVector3::CrossProduct(Y, UpCandidate).GetSafeNormal();
        const FVector3 Z = FVector3::CrossProduct(X, Y).GetSafeNormal();
    ...
    static FMatrix MakeFromZ(const FVector3 &ZAxis) noexcept
    {
        const FVector3 Z = ZAxis.GetSafeNormal();
        if (Z.IsNearlyZero())
        {
            return Identity;
        }

        const FVector3 ForwardCandidate =
            (std::fabs(Z.X) < 0.999f) ? FVector3::ForwardVector : FVector3::RightVector;

        const FVector3 Y = FVector3::CrossProduct(Z, ForwardCandidate).GetSafeNormal();
        const FVector3 X = FVector3::CrossProduct(Y, Z).GetSafeNormal();
    ...
    static FMatrix MakeLookAt(const FVector3 &Eye, const FVector3 &Target,
                              const FVector3 &Up = FVector3::UpVector) noexcept
    {
        ...
        const FVector3 Right = FVector3::CrossProduct(Up, Forward).GetSafeNormal();
        if (Right.IsNearlyZero())
        {
            return Identity;
        }

        const FVector3 NewUp = FVector3::CrossProduct(Forward, Right).GetSafeNormal();
    ...
    static FMatrix MakeViewLookAtLH(const FVector3 &Eye, const FVector3 &Target,
                                    const FVector3 &Up = FVector3::UpVector) noexcept
    {
        const FVector3 Forward = (Target - Eye).GetSafeNormal();
        if (Forward.IsNearlyZero())
        ...
        return FMatrix(Forward.X, Right.X, NewUp.X, 0.f, Forward.Y, Right.Y, NewUp.Y, 0.f,
                       Forward.Z, Right.Z, NewUp.Z, 0.f, -FVector3::DotProduct(Eye, Forward),
                       -FVector3::DotProduct(Eye, Right), -FVector3::DotProduct(Eye, NewUp), 1.f);
    }

    // 지정한 위치에서 카메라를 바라보는 Billboard 행렬을 생성함
    static FMatrix MakeBillboard(const FVector3 &Position, const FVector3 &CameraPosition,
                                 const FVector3 &Up = FVector3::UpVector) noexcept
    {
        const FVector3 Forward = (CameraPosition - Position).GetSafeNormal();
        if (Forward.IsNearlyZero())
        {
    ...
    static FMatrix MakeWorld(const FVector3 &Translation, const FMatrix &RotationMatrix,
    ...
    static FMatrix MakeTRS(const FVector3 &Translation, const FMatrix &RotationMatrix,
                           const FVector3 &Scale) noexcept
    {
...
// ---- FDXMathBridge inline implementations for FMatrix ----
namespace FDXMathBridge
{
    inline MatrixRegister LoadMatrix(const FMatrix &M) noexcept
    {
        return DirectX::XMMATRIX(M.M[0][0], M.M[0][1], M.M[0][2], M.M[0][3], M.M[1][0], M.M[1][1],
                                 M.M[1][2], M.M[1][3], M.M[2][0], M.M[2][1], M.M[2][2], M.M[2][3],
