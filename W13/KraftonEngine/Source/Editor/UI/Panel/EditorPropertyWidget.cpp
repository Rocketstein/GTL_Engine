...
{
	// 실제 위젯 렌더링은 공용 FPropertyTable 로 위임(SoftObject 에셋 피커 포함 전부 내장). 액터
	// 패널 고유 상태인 "편집 중 SceneComponent 의 회전 캐시 적용"만 컨텍스트 콜백으로 주입한다.
	FPropertyTable::FContext Ctx;
	if (SelectedComponent && SelectedComponent->IsA<USceneComponent>())
	{
		USceneComponent* SceneComp = static_cast<USceneComponent*>(SelectedComponent);
		Ctx.OnRotatorEdited = [SceneComp]() { SceneComp->ApplyCachedEditRotator(); };
	}
	return FPropertyTable::RenderValue(Props, Index, Ctx, bDispatchChange, PropertyPath);
}
