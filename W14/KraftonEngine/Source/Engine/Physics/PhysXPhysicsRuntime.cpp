...
void FPhysXPhysicsRuntime::AddImpulseAtLocation(
    FPhysicsBodyHandle BodyHandle,
...
{
    FPhysicsCommand Cmd;
    Cmd.Type         = EPhysicsCommandType::AddImpulseAtLocation;
    Cmd.Body         = BodyHandle;
    Cmd.VectorValue  = Impulse;
    Cmd.VectorValue2 = WorldLocation;
    EnqueueCommand(Cmd);
...
void FPhysXPhysicsRuntime::ApplyAddImpulseAtLocation_Internal(
    FPhysicsBodyHandle BodyHandle,
    const FVector&     Impulse,
    const FVector&     WorldLocation
)
{
    ...
    // Off-center impulse → torque about COM, so the body rotates about its joint.
    PxRigidBodyExt::addForceAtPos(
        *Dynamic,
        ToPxVec3(Impulse),
        ToPxVec3(WorldLocation),
...
int32 FPhysXPhysicsRuntime::ApplyCommands(const TArray<FPhysicsCommand>& Commands, float ContinuousForceDurationSeconds)
{
    ...
    for (const FPhysicsCommand& Command : Commands)
    {
        ...
        case EPhysicsCommandType::AddImpulse:
            ...
            break;
        case EPhysicsCommandType::AddImpulseAtLocation:
            ApplyAddImpulseAtLocation_Internal(Command.Body, Command.VectorValue, Command.VectorValue2);
            break;
