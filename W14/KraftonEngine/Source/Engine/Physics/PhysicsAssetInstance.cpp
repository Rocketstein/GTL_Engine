...
bool FPhysicsAssetInstance::AddImpulseToBoneAtLocation(const FName& BoneName, const FVector& Impulse, const FVector& WorldLocation) const
{
    ...
    Runtime->AddImpulseAtLocation(BodyHandle, Impulse, WorldLocation);
    return true;
}

bool FPhysicsAssetInstance::AddImpulseToBoneAtOffset(const FName& BoneName, const FVector& Impulse, const FVector& LocalOffset) const
{
    // Offset is in the body's current world frame; PhysX measures the lever arm from COM.
    const FTransform BodyWorld = GetBodyWorldTransformByBoneName(BoneName);
    const FVector WorldLocation = BodyWorld.Location + BodyWorld.Rotation.RotateVector(LocalOffset);
    return AddImpulseToBoneAtLocation(BoneName, Impulse, WorldLocation);
}
