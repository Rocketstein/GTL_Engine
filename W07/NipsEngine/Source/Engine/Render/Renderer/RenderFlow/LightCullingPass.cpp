...
bool FLightCullingPass::DrawCommand(const FRenderPassContext* Context)
{
	...
	if (CullMode == ELightCullMode::Clustered)
	{
		FComputeShader* DepthMaskCS = FResourceManager::Get().GetComputeShader("LightTileDepthMaskCS");
		if (!DepthMaskCS) {
			return false;
		}

		DepthMaskCS->Bind(DeviceContext);

		ID3D11ShaderResourceView* DepthSRV = RenderTargets->SceneDepthSRV;
		DeviceContext->CSSetShaderResources(0, 1, &DepthSRV);

		ID3D11UnorderedAccessView* DepthMaskUAV = Resources->LightTileDepthMask.GetUAV();
		DeviceContext->CSSetUnorderedAccessViews(0, 1, &DepthMaskUAV, nullptr);
		DepthMaskCS->Dispatch(DeviceContext, groupsX, groupsY, 1);
		DepthMaskCS->Unbind(DeviceContext);
	...
	CullingCS->Bind(DeviceContext);
	ID3D11ShaderResourceView* SRVs[] = {
		RenderTargets->SceneDepthSRV,
		Resources->LightTileDepthMask.GetSRV(),
		Resources->LightStructuredBuffer.GetSRV()
	};
	DeviceContext->CSSetShaderResources(0, 2, &SRVs[0]);
	DeviceContext->CSSetShaderResources(4, 1, &SRVs[2]);
    ...
    ID3D11ShaderResourceView* nullSRVs[] = { nullptr, nullptr, nullptr };
    DeviceContext->CSSetShaderResources(0, 2, &nullSRVs[0]);
    DeviceContext->CSSetShaderResources(4, 1, &nullSRVs[2]);
