...
StructuredBuffer<uint> TileDepthMask : register(t1);
#endif
...
groupshared float TileWeights[MAX_LIGHTS_SCRATCH];
groupshared float TileDistanceSq[MAX_LIGHTS_SCRATCH];
...
void main(uint3 groupID : SV_GroupID, uint3 groupThreadID : SV_GroupThreadID, uint threadIndex : SV_GroupIndex)
{
    ...
    if ((TileDepthMask[flatTileIndex] & (1u << groupID.z)) == 0)
    {
        if (threadIndex == 0)
            TileBuffer[flatClusterIndex] = uint2(storageOffset, 0);
        return;
    }
    ...
    for (uint i = threadIndex; i < LightCount; i += TILE_SIZE * TILE_SIZE)
    {
        ...
        if (index < MAX_LIGHTS_SCRATCH)
        {
            ...
            float3 delta = CameraPosition - light.Position;
    ...
    if (threadIndex == 0)
    {
        ...
        // Sort by weight, then distance, then light index, so the selected lights are deterministic.
        for (uint i = 1; i < scratchCount; i++)
        {
            float keyWeight = TileWeights[i];
            float keyDistanceSq = TileDistanceSq[i];
            uint keyIdx = TileLightIndices[i];
            int j = (int) i - 1;
        
            while (j >= 0 &&
                (TileWeights[j] < keyWeight ||
                (TileWeights[j] == keyWeight &&
                (TileDistanceSq[j] > keyDistanceSq ||
                (TileDistanceSq[j] == keyDistanceSq && TileLightIndices[j] > keyIdx)))))
            {
                TileWeights[j + 1] = TileWeights[j];
                TileDistanceSq[j + 1] = TileDistanceSq[j];
                TileLightIndices[j + 1] = TileLightIndices[j];
                j--;
            }
        
            TileWeights[j + 1] = keyWeight;
            TileDistanceSq[j + 1] = keyDistanceSq;
